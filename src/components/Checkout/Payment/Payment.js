...
const Payment = ({token, handleCaptureCheckout}) => {
   ...
   /*  console.log("Shipping_Country ---->", Shipping_Country)
    ...
    return (
        <>
            <Summary token={token}/>
            <Typography variant="h6" gutterBottom style={{ margin: '20px 0' }}>Betalnings metod</Typography>
                <Elements stripe={stripePromise}>
                    ...
                    <form onSubmit={(e) => handleSubmit(e, elements, stripe)}>
                       ...
                       {/*  <PayPalScriptProvider options={{ "client-id": "test" }}>
                        ...
                        <Button variant="outlined" onClick={() =>setNext_step(0)}>Tillbaka</Button>
                      { pay ? <CircularProgress/> : <Button type="submit" variant="contained" disabled={!stripe}  color="primary">
                            Bekräfta {token.live.subtotal.raw} $
                        </Button>}
                        </div>
                    </form>
                    )}
